...
Once you have created a metered feature, you can start sending events to Flexprice. Events are JSON payloads that describe usage activity and are sent via HTTP POST requests.
...
func sendEvent() error {
    ...
    fmt.Printf("Event sent with status: %d\n", resp.StatusCode)
    return nil
}
```
...
function sendEvent() {
    ...
    echo "Event sent with status: $httpCode\n";
...
When sending many events, use the bulk endpoint:
...
Add useful properties for debugging and analysis:
...
1. [Validate Events](Validating%20events.md) - Check that events are being processed correctly
2. [Connect to Billing](Connecting%20to%20billing.md) - Set up pricing and see usage in invoices
...
- Check that external_ccustomer_id matches exactly
...
- Check the [Event Debugger](Event%20debugger.md) for processing status
