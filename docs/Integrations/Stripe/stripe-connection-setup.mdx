...
This guide walks you through setting up a Stripe connection in Flexprice to enable payment processing, customer synchronization, and webhook integration.
...
|------------|------------------------------|----------|---------|
| **Publishable Key** | Developers → API Keys | ✅ | Frontend integration (client-side) |
...
| **Account ID** | Settings → Account Details | ❌ | For Stripe Connect (optional) |
...
3. **Account ID (Optional):**
   ...
   - Find your Account ID under Account Details
   - Starts with `acct_`
...
```
https://your-domain.com/api/v1/webhooks/stripe/{tenant_id}/{environment_id}
```

**Example:**
```
https://api.flexprice.io/api/v1/webhooks/stripe/tenant_123/prod
```
...
1. **Use Test Keys**: Replace live keys with test keys
   - `pk_test_...` instead of `pk_live_...`
   - `sk_test_...` instead of `sk_live_...`

2. **Test Webhook Endpoint**: Use a tool like ngrok for local testing
   ```
   https://your-ngrok-url.ngrok.io/api/v1/webhooks/stripe/tenant_123/test
   ```
...
```json
{
  ...
  "details": {
    ...
    "webhook_endpoint": "https://api.stripe.com/v1/webhook_endpoints/we_123"
  }
}
```
...
For detailed API documentation, see the [API Reference](/api-reference).
