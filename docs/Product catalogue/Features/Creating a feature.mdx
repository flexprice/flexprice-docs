...
* Define Event Details
  ...
  * Name**Example** Suppose you want to track how many tokens users consume while calling your AI model. In that case:
  ...
  * **Filters** *(Optional)*&#x49;t is important to note that filters cannot be edited later, so decide up front if you’ll need them for billing or reporting.**Example** Continuing our AI tokens example: If your system supports multiple AI models (**`gpt 3`**, **`llama3.2`**,**`gpt 4`**), and you only want to track usage from gpt models for specialized billing, you can add a filter:This means Flexprice will only aggregate token usage events where **`model`** is either `gpt 3` or  **`gpt 4`**
