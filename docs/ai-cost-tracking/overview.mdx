...
flowchart TB
  subgraph sources ["AI sources"]
    G["Gateways: LiteLLM, OpenRouter, Portkey"]
    O["Observability: Langfuse, Helicone"]
    C["Clouds: AWS Bedrock, Databricks, Snowflake"]
    S["SaaS AI: Agentforce, SAP Joule"]
  end

  subgraph ingest ["Ingestion"]
    COL["Flexprice Collector (push / in-infra)"]
    PULL["Managed pull (Flexprice-hosted, scheduled)"]
  end

  subgraph fp ["Flexprice"]
    EV["Standardised AI usage events"]
    METER["Metering and model pricing"]
    GOV["Wallets, credits, alerts and limits"]
    AN["Cost, token and margin analytics"]
    EV --> METER --> GOV --> AN
  end
  ...
  C --> PULL
  S --> PULL
  G -. "real-time push" .-> PULL
  COL --> EV
  PULL --> EV

  GOV -. "alerts and enforcement" .-> sources
```
