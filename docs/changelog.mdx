...
<Update label="April 13th 2026">
  ...
  * **Validation**: The minimum billing period count check no longer applies to one-time prices
  ...
  ## AI-assisted pricing setup

  Paste a pricing page or document and Flexprice uses Gemini to extract structured prices — reducing manual entry when onboarding a new product catalog.

  * **Gemini proxy endpoint**: `POST /v1/ai/pricing/parse` accepts raw pricing text and returns structured price objects ready for import, with retry logic for JSON parsing failures
  * **Dashboard wizard**: The pricing setup flow includes an AI step — upload or paste pricing content and review the extracted prices before saving
  * **Model configuration**: The Gemini model version is configurable; defaults to `gemini-2.5-flash-lite`
  ...
  **Other changes**
  ...
  <AccordionGroup>
    ...
    <Accordion title="API">
      * New `POST /v1/ai/pricing/parse` endpoint for Gemini-backed pricing extraction from raw text
