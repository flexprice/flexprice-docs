...
<Update label="February 12th 2026">
  ## Usage Ingestion & Stability Improvements

  Stronger event ingestion reliability under high throughput and improved validation to prevent aggregation edge cases.

  * **Event ingestion handling**: Strengthened event ingestion handling to improve reliability under high-throughput usage workloads.
  * **Billable metric validation**: Improved internal validation around billable metric processing to prevent incorrect aggregation edge cases.

  <br />

  ## Subscription & Pricing Consistency

  Consistent subscription state across plan updates and scheduled transitions, with safer pricing configuration to reduce invalid state transitions.

  * **Subscription state handling**: Enhanced subscription state handling to ensure consistent behavior across plan updates and scheduled transitions.
  * **Pricing configuration safeguards**: Improved pricing configuration safeguards to reduce risk of invalid state transitions during updates.

  <br />

  ## Workflow & Background Task Reliability

  More predictable Temporal execution and error propagation, with less failure in scheduled billing operations.

  * **Temporal workflow handling**: Improved Temporal workflow handling to ensure more predictable execution and error propagation.
  * **Scheduled billing operations**: Internal cleanup to reduce failure scenarios during scheduled billing operations.
  ...
  **Other changes**
  ...
  <AccordionGroup>
    <Accordion title="Improvements">
      * Refined internal logging context for better debugging in production environments.
      * Minor performance optimizations in wallet and balance calculation paths.
      * Improved internal validation around pricing and subscription payload structures.
    </Accordion>

    <Accordion title="Fixes">
      * Fixed edge cases in usage aggregation under specific filtering conditions.
      * Addressed minor inconsistencies in subscription lifecycle handling.
      * Resolved race condition scenarios in concurrent billing operations.
    </Accordion>

    <Accordion title="API">
      * Minor API stability improvements to subscription and pricing endpoints.
      * Improved error response consistency across billing-related endpoints.
    </Accordion>
  </AccordionGroup>
</Update>
