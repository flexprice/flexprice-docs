...
<Update label="May 18th 2026">
  ...
  * **Dashboard support**: Set `auto_invoice_threshold` during subscription creation; the current threshold value is shown on the subscription detail page
  ...
  ## White-label dashboard & internationalization

  The Flexprice frontend now ships with full white-label support and a first-class RTL internationalization layer, including Arabic locale strings across all dashboard namespaces.

  * **Brand & auth config**: `BrandConfig`, `AuthPageConfig`, and `I18nConfig` in `config.ts` drive per-deployment branding — logo, tagline, landing colors, allowed locales — without code changes
  * **Auth templates**: `Template2` provides a custom-deployment auth page with configurable logo, tagline, and landing panel colors; `FlexpriceDefault` (Template1) remains the default
  * **RTL layout**: Sidebar, drawers, breadcrumbs, table alignment, and all directional CSS classes are locale-direction-aware using logical CSS properties
  * **Locale store**: `useLocaleStore` (Zustand + persist) persists locale selection across sessions, updates `document.dir`, and calls `i18next.changeLanguage` at runtime
  * **Arabic locale**: Full Arabic locale strings added across all dashboard namespaces; automated `extract-i18n.mjs` script and an ESLint guard blocks new hardcoded user-visible strings
