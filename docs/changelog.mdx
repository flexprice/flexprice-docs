...
<Update label="September 14th 2026">
  ...
  One-off invoices can now be gated behind a hosted payment. The invoice is created as a draft and finalizes only when the payment succeeds.

  * **Opt in per request**: Add a `checkout` object to `POST /invoices`. Omit it and the endpoint behaves exactly as before
  * **One-off only**: Gated invoices must be `ONE_OFF`, and `invoice_status`, `payment_status`, `amount_paid`, and `total_prepaid_applied` are rejected because the checkout session sets them on payment
  * **Ownership guards**: Only the checkout session that owns an invoice can finalize or void it while that session is pending; every other caller is held off until the session is terminal
  * **Expiry handling**: An expired session voids the invoice when it already holds payment or applied prepaid credits, so that value is returned instead of archived away
  * **Session creation**: `pay_invoice` sessions are created through invoice creation only. Passing the action to `POST /checkout/sessions` returns a validation error pointing at the right endpoint

  <br />

  <Card title="Pay-first checkout - Documentation" icon="book-open" horizontal href="/docs/checkout/pay-first-checkout">
  </Card>

  <br />

  ## Custom currency configuration in the dashboard

  Custom currencies can now be defined from Settings rather than through the settings API.

  - **Configuration**: Choose the fiat settlement currency, then add each custom currency with a code, name, symbol, and a conversion factor per fiat currency
  - **Across the app**: Custom currency symbols render in coupons, invoice creation, the revenue trend card, and the currency selector
  - **Validation**: Codes are case-insensitive and must be 3 characters, cannot reuse the default fiat currency code, and every currency must define a factor for the settlement currency
  - **Access**: The section follows the other billing settings and is limited to Super Admins with setting write permission

  <br />

  <Card title="Custom currency - Documentation" icon="book-open" horizontal href="/docs/product-catalogue/custom-pricing/custom-currency">
  </Card>
  ...
  Usage and fixed charges can now be entered as a percentage in the dashboard.

  * **Entry and display**: Enter `2.5` and the charge reads back as `2.5%` in charge tables, previews, tooltips, override summaries, and the pricing card, with no currency symbol and no per-unit suffix
  * **Storage**: The charge persists as a `FLAT_FEE` whose amount is the decimal equivalent, tagged with `metadata.billing_model = percentage`, so the API shape is unchanged
  * **Conversion**: Percent to decimal shifts the decimal point instead of dividing by 100, so a value such as `1.1` does not pick up floating point error before it is stored
  * **Editing**: The update price and override dialogs now edit in percent and convert on save, rather than writing the stored decimal back as a raw amount
  * **Model switches**: Switching a charge to package or tiered clears the percentage marker, so tiers are never read as percentages

  <br />

  ## Line item grouping on subscription create

  A charge whose cadence is shorter than the subscription's can now bill as a single line item for the billing period instead of one per charge period.

  - **New field**: `line_item_grouping` on `POST /subscriptions` accepts `PER_CHARGE_PERIOD`, the default, where a monthly charge on a quarterly subscription bills as 3 rows, or `PER_BILLING_PERIOD`, where those rows collapse into 1
  - **Totals unchanged**: Charges are still computed per charge period, so the invoice total is identical under either value
  - **Dashboard toggle**: The create form shows the control only when an attached charge will actually split, and re-checks that condition on submit
  - **Commitment overage**: Subscriptions with cumulative commitment overage keep their plan level overage rows unmerged, and the section says so when a commitment is configured
  ...
  **Other changes**
  ...
  <AccordionGroup>
    <Accordion title="Improvements">
      * **Stripe price sync**: Stripe products are keyed on the root price, so every subscription override of a plan price shares one catalog item instead of minting a duplicate
      * **Zoho invoice sync**: Recorded payments accept a payment mode and a "Deposit To" account id, and global custom fields write fixed values to Zoho invoice custom fields on every sync
      * **Scheduled exports**: Task creation fails up front when the connection has no export configuration, validation errors are no longer retried, and GCS destinations no longer require a region
      * **Tax associations**: Tax behavior defaults to exclusive across tax associations
      * **Invoice editing**: Invoice details is now the editing surface, with an inline row editor and a clearable calendar
      * **Wallet alerts**: Balance thresholds use a compact settings list shared by wallet and tenant settings
      * **Subscription create**: Addon quantity can be changed while creating a subscription
      * **Invoice detail**: The reapply coupon control moved below the line items table, behind a divider
      * **Onboarding**: Tenant setup requires an organization URL
      * **Self-hosting**: The Helm chart runs a replicated ClickHouse cluster against an existing Keeper quorum through `clickhouse.altinity.replicasCount` and `clickhouse.altinity.keeper.hosts`, and spreads replicas across nodes by default. Chart defaults render the same single-replica cluster as before
      * **Self-hosting**: The worker autoscaler also scales on memory, and the pod disruption budget lets single-replica pods be evicted
    </Accordion>

    <Accordion title="Fixes">
      * **Multi-cadence proration**: Prorations no longer skip on subscriptions with mixed cadences, basis items fan out on the charge's own cadence, and the denominator uses the matching period
      * **Immediate cancellation**: Cancelling immediately closes the open period, clamps to the period start, and restores the pre-cancellation period on the local copy
      * **Invoice line items**: Merged line items keep the fields they were dropping, and `parent_line_item_id` persists through bulk line item creates
      * **Credit expiry**: Held credits stay unexpired until the invoice is finalized
      * **Draft subscriptions**: Their invoices are referenced by the period start
      * **Usage analytics export**: Results are no longer grouped by source
      * **Scheduled export destination**: Bucket, region, and encryption resolve from the run's job config rather than the connection, so one bring-your-own-bucket connection can run tasks with different destinations
      * **Paddle**: Charges no longer rewrite the catalog on every invoice sync, and a not-found response no longer surfaces as a 5xx
      * **Razorpay**: Customer creation is idempotent
      * **Zoho**: One-time billing period sync corrected
      * **HubSpot**: Integration sync corrected
      * **Subscription cancellation**: Line item dates are checked before cancellation
      * **Plan filter**: The plan id filter returns the right results
      * **Dashboard dark mode**: The update toast and the Add Credits dialog are readable
      * **Nested dialogs**: Escape dismisses only the dropdown it was opened on, instead of closing the dialog underneath it and losing the form
      * **Addon charges**: The Add Subscription table shows the actual, possibly overridden, addon charge
      * **Exports dialog**: The manual export date picker opens inside the dialog
      * **Spend alerts**: Dialog labels read from the `spendAlerts` namespace
    </Accordion>

    <Accordion title="API">
      * **Gated invoices**: `POST /invoices` accepts a `checkout` object to gate a one-off invoice behind payment, and `pay_invoice` is rejected on `POST /checkout/sessions`
      * **Line item grouping**: `POST /subscriptions` accepts `line_item_grouping`, either `PER_CHARGE_PERIOD` or `PER_BILLING_PERIOD`
      * **New webhook event**: `subscription.plan_changed` fires when a subscription plan changes in place, with the subscription id and billing anchor preserved
      * **Subscription payloads**: Subscription webhook payloads include `invoicing_customer_id`
      * **Subscription schedules**: The subscription schedule endpoints are now published in the OpenAPI spec and the generated SDKs
      * **Custom currency validation**: Custom currency codes must be 3 characters, and the default fiat currency cannot also be a custom currency code
    </Accordion>
  </AccordionGroup>
</Update>
...
<Update label="September 7th 2026">
  ...
  * **Portal updates**: The Overview tab brings together wallet balance, payment methods, and subscriptions; payment flows open in a new tab and refresh the portal on completion
  ...
  <Card title="Customer portal - Documentation" icon="book-open" horizontal href="/docs/customers/customer-portal">
  </Card>
  ...
  * **Manual edit protection**: Edited invoices are marked with `is_manually_edited` so automated recomputation cannot overwrite them; line item updates create a new item ID
  ...
  <Card title="Execute invoice modification - API Reference" icon="code" horizontal href="/api-reference/invoices/execute-invoice-modification">
  </Card>
  ...
  * **More webhook states**: Balance alerts now emit warning and info transitions, plus recovery events when the balance returns to `ok`
  ...
  <Card title="Low balance alerts - Documentation" icon="book-open" horizontal href="/docs/wallet/low-balance-alert">
  </Card>
  ...
  * **Itemized checkout**: Checkout shows individual line items and handles zero-decimal currencies correctly
  ...
  <Card title="Chargebee invoice sync - Documentation" icon="book-open" horizontal href="/integrations/chargebee/invoice-sync">
  </Card>
...
<Update label="August 31st 2026">
  ...
  * **Percentage-only rates**: `percentage_value` is now required; `fixed_value` has been removed
  ...
  <Card title="Taxes - Documentation" icon="book-open" horizontal href="/docs/product-catalogue/taxes/overview">
  </Card>
  ...
  * **Proration control**: Set `proration_behavior: none` to add the full quota
  ...
  <Card title="Entitlement grants - Documentation" icon="book-open" horizontal href="/docs/product-catalogue/features/entitlement-grants">
  </Card>
  ...
  * **Configuration change**: `sync_config.s3` is now `sync_config.storage`
  ...
  <Card title="Self-hosting - Documentation" icon="book-open" horizontal href="/docs/getting-started/self-hosting-guide">
  </Card>
  ...
  <Card title="Marketplace integrations - Documentation" icon="book-open" horizontal href="/integrations/marketplace-integration/overview">
  </Card>
  ...
  <Card title="Remove user from tenant - API Reference" icon="code" horizontal href="/api-reference/users/remove-user-from-tenant">
  </Card>
...
<Update label="August 24th 2026">
  ...
  * **Grant updates**: Plan grants and entitlements switch at the change instant, preserving addon-sourced grants
  ...
  <Card title="Upgrade and downgrade - Documentation" icon="book-open" horizontal href="/docs/subscriptions/upgrade-and-downgrade">
  </Card>
  ...
  * **Immediate access checks**: Permissions load at login without needing a page refresh
  ...
  <Card title="RBAC - Documentation" icon="book-open" horizontal href="/docs/rbac/overview">
  </Card>
  ...
  * **Customer updates**: Changes to a Flexprice customer update the linked Zoho contact
  ...
  <Card title="Zoho Books - Documentation" icon="book-open" horizontal href="/integrations/zohobooks/connection-setup">
  </Card>
...
<Update label="August 17th 2026">
  ...
  * **Grant restrictions**: Admins can grant only permissions they hold, and invitations require Super Admin
  ...
  <Card title="Manage roles - Documentation" icon="book-open" horizontal href="/docs/rbac/manage-roles">
  </Card>
  ...
  * **Deployment setup**: Enable `auth.saml.enabled` with Redis and an auth signing secret; tenant login links show the SSO button
  ...
  <Card title="Execute a plan change (v2) - API Reference" icon="code" horizontal href="/api-reference/subscriptions/execute-a-plan-change-v2-swap-in-place">
  </Card>
  ...
  * **Dashboard toggle**: Enable Prices in the Stripe connection drawer; outbound price sync is off by default
  ...
  <Card title="Stripe invoice sync - Documentation" icon="book-open" horizontal href="/integrations/stripe/invoice-sync">
  </Card>
  ...
  <Card title="Exportable UI - Documentation" icon="book-open" horizontal href="/docs/exportable-ui/overview">
  </Card>
...
<Update label="August 10th 2026">
  ...
  * **Configurable behavior**: New checkout configuration fields control payment gating on subscription create requests
  ...
  <Card title="Checkout - Documentation" icon="book-open" horizontal href="/docs/checkout/overview">
  </Card>
  ...
  <Card title="Managing invoices - Documentation" icon="book-open" horizontal href="/docs/invoices/managing">
  </Card>
  ...
  * **Cache-hit safety**: Idempotent cache hits no longer re-publish webhooks or re-run finalization
  ...
  <Card title="Credit notes - API Reference" icon="code" horizontal href="/api-reference/introduction">
  </Card>
...
<Update label="August 3rd 2026">
  ...
  * **Post-insert alerts**: Usage alert evaluation can now run immediately after bulk inserts to keep threshold notifications timely
  ...
  <Card title="Bulk event ingestion - API Reference" icon="code" horizontal href="/api-reference/introduction">
  </Card>
  ...
  * **Cooloff on the dashboard**: The wallet auto top-up modal now accepts a cooloff duration and unit so repeat charges are spaced out
  ...
  <Card title="Wallet auto top-up - Documentation" icon="book-open" horizontal href="/docs/wallet/auto-top-up">
  </Card>
  ...
  * **Consistent shape**: Response matches the standard subscription payload used elsewhere in the API
  ...
  <Card title="Customers - API Reference" icon="code" horizontal href="/api-reference/endpoint/customer">
  </Card>
...
<Update label="July 27th 2026">
  ...
  * **Shared usage pipeline**: Backed by a refactored `UsageRecord` schema and cron workflow shared across all three marketplace providers
  ...
  <Frame>
    <img
      src="/images/docs/marketplace/gcp/gcp-connection-setup.png"
      alt="Connect to GCP Marketplace from the Flexprice dashboard"
  ...
  <Card title="GCP Marketplace - Documentation" icon="book-open" horizontal href="/integrations/marketplace-integration/gcp/connection-setup">
  </Card>
  ...
  * **Secret redaction**: A shared `RedactSecrets` utility sanitizes error messages from AWS, GCP, and Azure integrations so failure reasons stay visible without leaking credentials
  ...
  <Frame>
    <img
      src="/images/docs/marketplace/azure/creat-conn.png"
      alt="Connect to Azure Marketplace from the Flexprice dashboard"
      style={{ borderRadius: '0.5rem' }}
    />
  </Frame>
  ...
  <Card title="Azure Marketplace - Documentation" icon="book-open" horizontal href="/integrations/marketplace-integration/azure/connection-setup">
  </Card>
...
<Update label="July 20th 2026">
  ...
  * **Dashboard connection management**: A new AWS Marketplace connection drawer lets you configure and manage the connection directly from the dashboard, with improved UI and localization
  ...
  <Frame>
    <img
      src="/images/docs/marketplace/aws/bills.png"
      alt="AWS Bills page showing a Marketplace charge reported by Flexprice"
      style={{ borderRadius: '0.5rem' }}
    />
  </Frame>
  ...
  <Card title="AWS Marketplace - Documentation" icon="book-open" horizontal href="/integrations/marketplace-integration/aws/connection-setup">
  </Card>
  ...
  <Card title="Grouped invoicing - Documentation" icon="book-open" horizontal href="/docs/subscriptions/billing-workflows/grouped-invoicing">
  </Card>
...
<Update label="July 13th 2026">
  ...
  * **Dashboard support**: A new alert settings dialog lets you configure and save spend-alert thresholds for a subscription or line item directly from the dashboard
  ...
  <Card title="Spend alerts - Documentation" icon="book-open" horizontal href="/docs/subscriptions/how-to-configure-spend-alerts/what-are-spend-alerts">
  </Card>
  ...
  * **Reconciliation and webhook handling**: A reconciliation sweep recovers stuck charge claims, and the Razorpay webhook handler now processes `token.confirmed`, `token.rejected`, and `token.cancelled` events
  ...
  <Card title="Autopay & Mandates - Documentation" icon="book-open" horizontal href="/docs/checkout/autopay-mandates">
  </Card>
...
<Update label="July 6th 2026">
  ...
  * **Paddle event coverage**: Paddle connections can now subscribe to the full set of Paddle webhook event types, including `transaction.completed`
  ...
  <Card title="Webhooks - Documentation" icon="book-open" horizontal href="/docs/webhook/webhooks">
  </Card>
  ...
  * **Customer Portal tab**: Toggle which sections and visibility options are exposed on the customer-facing portal
  ...
  <Card title="Settings - Documentation" icon="book-open" horizontal href="/docs/settings/settings">
  </Card>
...
<Update label="June 29th 2026">
  ...
  * **Automatic expiry**: A Temporal workflow sweeps and expires stale sessions every 30 minutes so abandoned checkouts don't linger
  ...
  <Card title="Checkout Sessions API - Documentation" icon="book-open" horizontal href="/docs/checkout/checkout-sessions">
  </Card>
  ...
  * **Dashboard**: Subscription and customer timezone are now visible in the Customer Information and Subscription Details views
  ...
  <Card title="Customer Timezones - Documentation" icon="book-open" horizontal href="/docs/customers/timezone-aware-billing">
  </Card>
  ...
  <Frame>
    <img
      src="/images/docs/product-catalogue/features/custom-expression/switch-from-field-to-cel.png"
      alt="Aggregation section with the Custom expression toggle highlighted"
      style={{ borderRadius: '0.5rem' }}
    />
  </Frame>
  ...
  <Card title="Custom Expression - Documentation" icon="book-open" horizontal href="/docs/product-catalogue/features/custom-expression">
  </Card>
  ...
  <Frame>
    <img
      src="/images/docs/product-catalogue/features/creating-a-feature/add-entitlement-feat-config.png"
      alt="Add Entitlement with Config feature JSON editor"
      style={{ borderRadius: '0.5rem' }}
    />
  </Frame>
  ...
  <Card title="Creating a Feature - Documentation" icon="book-open" horizontal href="/docs/product-catalogue/features/create">
  </Card>
...
<Update label="June 22nd 2026">
  ...
  * **Cron reconciliation and void/refund lifecycle**: A scheduled workflow reconciles pending card-verification charges — voiding the verification charge automatically, or issuing a refund if the void fails
  ...
  <Frame>
    <img
      src="/images/docs/integrations/moyasar/setup-autopay.png"
      alt="Setup Autopay button on the customer page"
      style={{ borderRadius: '0.5rem' }}
    />
  </Frame>
  ...
  <Card title="Moyasar autopay - Documentation" icon="book-open" horizontal href="/integrations/moyasar/autopay">
  </Card>
  ...
  <Card title="Apply coupons to subscriptions - Documentation" icon="book-open" horizontal href="/docs/product-catalogue/coupons/apply-discount-on-subscription">
  </Card>

  <Card title="Apply taxes - Documentation" icon="book-open" horizontal href="/docs/product-catalogue/taxes/apply">
  </Card>
...
<Update label="June 15th 2026">
  ...
  <Card title="Service accounts - Documentation" icon="book-open" horizontal href="/docs/rbac/service-accounts/create">
  </Card>
  ...
  <Card title="Customer hierarchy - Documentation" icon="book-open" horizontal href="/docs/subscriptions/customer-hierarchy">
  </Card>
...
<Update label="March 30th 2026">
  ...
  * **Deferred credits and taxes**: Subscription invoice computation now applies only coupons during draft — credit deductions and tax calculations are deferred to finalization, preventing premature wallet debits
  ...
  <Card title="Invoices - Documentation" icon="book-open" horizontal href="/docs/invoices/overview">
  </Card>
  ...
  * **Dashboard support**: The subscription cancellation dialog in the dashboard now includes the scheduled cancellation option with a date picker
  ...
  <Card title="Subscriptions - Documentation" icon="book-open" horizontal href="/docs/subscriptions/view">
  </Card>
...
<Update label="March 23rd 2026">
  ...
  * **Enhanced error handling**: Multi-layer idempotency checks on invoice sync prevent duplicate Paddle transactions on retries
  ...
  <Card title="Paddle connection setup - Documentation" icon="book-open" horizontal href="/integrations/paddle/connection-setup">
  </Card>
  ...
  * **Flexible cancellation strategies**: Choose between immediate, minimum-period-end, or maximum-period-end cancellation to honor varying commitment lengths
  ...
  <Card title="Creating a plan - Documentation" icon="book-open" horizontal href="/docs/product-catalogue/plans/create">
  </Card>
  ...
  * **Sequential usage fetch**: New `GetUsageBulkSync` method provides reliable, sequential meter usage lookups with per-meter timeout protection for billing accuracy
  ...
  <Card title="Sending events - Documentation" icon="book-open" horizontal href="/docs/event-ingestion/sending-events">
  </Card>
...
<Update label="March 16th 2026">
  ...
  * **Logical feature groups**: Group related features together for better organization and clarity
  ...
  <Card title="Feature grouping - Documentation" icon="book-open" horizontal href="/docs/product-catalogue/groups/feature-grouping">
  </Card>
...
<Update label="March 2nd 2026">
  ...
  * **Clone plans**: Quickly create new plans based on existing configurations
  ...
  <Card title="Cloning a plan - Documentation" icon="book-open" horizontal href="/docs/product-catalogue/plans/clone">
  </Card>
  ...
  * **Group prices**: Organize related prices into logical groups
  ...
  <Card title="Charges grouping - Documentation" icon="book-open" horizontal href="/docs/product-catalogue/groups/charges-grouping">
  </Card>
...
<Update label="February 23rd 2026">
  ...
  * **Alert cache expiration**: Alert cache entries now expire after a configurable max-alive window to prevent stale suppression of alerts
  ...
  <Card title="Low balance alert - Documentation" icon="book-open" horizontal href="/docs/wallet/low-balance-alert">
  </Card>
...
<Update label="February 16th 2026">
  ...
  * **Subscription hierarchy**: Organize subscriptions with parent-child relationships
  ...
  <Card title="Update subscription - API Reference" icon="code" horizontal href="/api-reference/subscriptions/update-subscription">
  </Card>
  ...
  * **Dashboard configuration**: Configure and view commitment durations directly from the subscription management dashboard
  ...
  <Card title="Commitment - Documentation" icon="book-open" horizontal href="/docs/subscriptions/commitment/overview">
  </Card>
  ...
  * **Flexible add-on instances**: Support for multiple instances of the same add-on per subscription
  ...
  <Card title="Create add-ons - API Reference" icon="code" horizontal href="/api-reference/addons/create-addon">
  </Card>
...
<Update label="February 9th 2026">
  ...
  * **Edit existing grants**: Update credit grant details directly without voiding
  ...
  <Card title="Update credit grant - API Reference" icon="code" horizontal href="/api-reference/credit-grants/update-credit-grant">
  </Card>
  ...
  * **Wallet modes**: Configure wallets as prepaid or postpaid based on customer needs
  ...
  <Card title="Prepaid & promotional credits - Documentation" icon="book-open" horizontal href="/docs/wallet/prepaid-and-promotional">
  </Card>
...
<Update label="February 2nd 2026">
  ...
  * **Workflow optimizations**: Overall wallet workflow optimizations for performance and correctness
  ...
  <Card title="Get Subscription - API Reference" icon="code" horizontal href="/api-reference/subscriptions/get-subscription-v2">
  </Card>
...
<Update label="January 28th 2026">
  ...
  * **S3 exports**: Managed exports and downloadable reports for billing and credit usage
  ...
  <Card title="S3 Export - Documentation" icon="book-open" horizontal href="/docs/data-exports/amazon-s3/export">
  </Card>
...
<Update label="January 21st 2026">
  ...
  * **Advisory locking**: Wallet operations are protected with advisory locking to reduce race conditions under concurrency
  ...
  <Card title="Get Wallet Balance - API Reference" icon="code" horizontal href="/api-reference/wallets/get-wallet-balance">
  </Card>
