...
<Update label="September 14th 2026">
  ...
  * **Dashboard**: Configure currencies from Settings under Billing, and pick them from the currency selectors for charges, coupons, invoices, and wallets
  ...
  One-off invoices can now be created behind a hosted checkout session, and checkout sessions can be cancelled and polled for their result.
  ...
  * **Cancel endpoint**: `POST /checkout/sessions/{id}/cancel` ends an in-flight session and cleans up the draft records created for it. Completed sessions cannot be cancelled
  ...
  * **Dashboard**: The create subscription form shows an invoice line items toggle whenever a charge is shorter than the billing period; the setting is written at create time
  ...
  ## Tax behavior in the dashboard

  Tax behavior, tax treatment, and the invoice tax breakdown are now available in the dashboard, and the default behavior is exclusive for every currency.

  * **Default change**: Tax associations created without a `tax_behavior` now default to `exclusive` regardless of currency. Previously USD and CAD defaulted to exclusive and other currencies to inclusive. Associations stored without a behavior read back as exclusive
  * **Tax associations**: Choose exclusive or inclusive when applying a tax rate, and see the behavior in tax association tables
  * **Tax treatment**: Mark a customer as taxable or exempt when creating a customer or from the customer information tab
  * **Invoice breakdown**: Invoice and subscription preview line item tables show inclusive tax, exclusive tax, total tax, and the reason when no tax was charged
  ...
  **Other changes**
  ...
  <AccordionGroup>
    <Accordion title="Improvements">
      ...
      * The invoice edit page uses the invoice details as the editing surface, with inline row editing for line items and a clear action on the calendar
    ...
    <Accordion title="API">
      ...
      * Subscription schedule `configuration` and `execution_result` are open objects in the OpenAPI spec so SDKs keep their keys, and the list endpoint accepts `subscription_id`
    </Accordion>
  </AccordionGroup>
</Update>
