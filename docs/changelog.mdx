...
<Update label="September 14th 2026">
  ## Tenant custom currencies

  You can now define your own currencies, price your catalogue in them, and have Flexprice settle invoices in fiat.

  * **Configuration**: Define currencies in the `custom_currency_config` setting, each with a name, symbol, and a conversion factor per fiat currency, plus the `default_fiat_currency` invoices are denominated in
  * **Where they apply**: Prices, subscriptions, wallets, and addons can be created in a configured custom currency or in a mapped fiat currency; anything else is rejected with the supported codes listed
  * **Invoices**: Invoices and line items carry a `custom_currency` block alongside their fiat amounts, and the conversion factor is frozen at finalization so a later factor change cannot move a finalized invoice
  * **Validation**: Codes are three characters and case insensitive, every currency needs a factor for the same set of fiat currencies, and the default fiat currency cannot also be a custom code
  * **Dashboard**: Settings, Billing has a custom currency section for adding currencies and their conversion factors
  ...
  A one-off invoice can now be held behind a hosted payment, joining the seat change and wallet top-up flows that already accept a `checkout` object.

  - **How it works**: Pass `checkout` to `POST /invoices` and the invoice is created as a computed draft with a payment link attached; it finalizes only when the payment webhook lands
  - **Provenance**: Gated invoices carry `source_type: checkout`, and edits, voids, and finalization are blocked while the session is still active
  - **Guards**: The invoice type must be `ONE_OFF`, `amount_due` must be positive, and any `due_date` must be in the future so a draft nobody can pay yet is never marked overdue
  - **Zero-amount handling**: An invoice that computes to zero, or one fully covered by prepaid credits, is voided and archived rather than gated, and the credits are returned
  ...
  ## Checkout session reconciliation and Razorpay payment links

  Checkout sessions now reconcile against the payment provider when they are read, so a late or dropped webhook no longer leaves a paid session pending.

  - **Reconciliation on read**: Reading a non-terminal session checks the provider and completes the session if the customer has paid; every transition still runs through the same completion path, so a concurrent webhook cannot double-apply it
  - **Poll interval**: Session and portal payment reads return `next_poll_after_ms`, which starts at 2 seconds, widens to 5 seconds after 30 seconds, then settles at 10 seconds; zero means the session is finished
  - **Razorpay checkout**: Razorpay is available as a checkout provider for payment links and can read payment state back, resolving payment link, invoice, order, and payment handles
  - **Degradation**: A gateway that does not answer returns the stored state marked stale instead of an error

  <br />

  <Card title="Razorpay checkout - Documentation" icon="book-open" horizontal href="/docs/checkout/razorpay-checkout">
  </Card>

  <br />

  ## Line item grouping on subscriptions

  A charge whose cadence is shorter than its subscription can now bill as a single invoice line item instead of one per charge period.

  * **Setting**: `line_item_grouping` on subscription create takes `per_charge_period`, the existing behaviour and the default, or `per_billing_period`
  * **Presentation only**: Charges are still computed per charge period, so a monthly price on a quarterly subscription totals the same under either value and only the row count changes
  * **Dashboard**: The create subscription form shows a toggle, and only when at least one attached charge actually splits; a cumulative commitment keeps its plan-level overage rows separate, which the form calls out

  <br />

  ## Percentage fee charges in the dashboard

  Usage and recurring charges can now be entered as a percentage in the plan builder, for cases like a fee taken as a share of transaction value.

  - **Entry and display**: Enter `2.5` rather than `0.025`, with the conversion done on the string to avoid floating point drift; price overrides and the subscription price table read the value back as a percentage
  - **Storage**: This is a dashboard-level billing model. The charge is persisted as a `FLAT_FEE` price whose amount holds the decimal equivalent, tagged with `metadata.billing_model: percentage`. The API exposes no percentage billing model, and a price that is later switched to another billing model drops the tag
  ...
  **Other changes**
  ...
  <AccordionGroup>
    <Accordion title="Improvements">
      * Addon charges on a shorter cadence than their subscription now fan prorations out per charge period, so a mixed-cadence subscription no longer skips proration
      * Scheduled exports resolve their destination from the job configuration, make the region optional for GCS, fail task creation when no export destination is configured, and stop retrying validation errors
      * Usage analytics exports are no longer grouped by source, so commitment aggregates come out as one row
      * Invoice configuration exposes a finalization delay in the dashboard, setting how long after creation an invoice waits before it finalizes
      * Invoice details can hide zero-amount line items behind a toggle
      * Tenant onboarding asks for an organization URL
      * Addon quantity can be changed while a subscription is being created
      * Zoho Books payment sync records a payment mode and a "Deposit To" account, and syncs global custom fields
      * ClickHouse can be deployed as a replicated cluster backed by Keeper, and the worker autoscaler now also scales on memory
      * Tax behaviour defaults to exclusive across tax associations
    </Accordion>

    <Accordion title="Fixes">
      * Immediate subscription cancellation clamps to the period start and closes the open period instead of leaving it running
      * Subscription cancellation checks line item dates before it cancels
      * Credit expiry is held until the invoice is finalized, so credits are no longer expired against a draft
      * Merged invoice line items keep fields that were previously dropped, and `parent_line_item_id` survives bulk line item creation
      * Zoho invoices sync the correct billing period for one-time charges
      * Razorpay customer creation is idempotent, so a retry no longer creates a second customer
      * Paddle returns a 5xx on a missing object rather than a success, and no longer rewrites the catalogue on every invoice sync
      * HubSpot invoice sync sends the right invoice fields
      * The Add Credits dialog and the update toast are readable in dark mode
      * The manual export date picker opens inside its dialog instead of behind it
      * A slow sidebar navigation no longer overrides a faster click on a child item
      * Every role checkbox stays clickable when Super Admin is selected
      * A dialog opened from inside another dialog no longer closes the one beneath it
    </Accordion>

    <Accordion title="API">
      * `POST /invoices` accepts a `checkout` object for one-off invoices, and invoices carry a read-only `source_type`
      * Subscription create accepts `line_item_grouping` with `per_charge_period` or `per_billing_period`
      * Invoice and invoice line item responses include a `custom_currency` denomination, and `custom_currency_config` is a new setting key
      * Checkout session and customer portal payment reads return `next_poll_after_ms`
      * Razorpay is selectable as the `payment_provider` on checkout sessions
    </Accordion>
  </AccordionGroup>
</Update>
