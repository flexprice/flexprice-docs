...
<Warning>
  Set `contact` at customer-creation time. Adding it later does not
  retroactively fix a mandate registration that already failed — you will need
...
| `payment_method`    | Yes      | `UPI` for UPI Autopay, `CARD` for a card mandate.                                                                                   |
...
A card mandate uses the same request. Change `payment_method` to `CARD`:
...
<Note>
  ...
  enough to cover your largest expected invoice.
</Note>
