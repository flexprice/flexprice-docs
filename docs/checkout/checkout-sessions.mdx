...
| `next_poll_after_ms` | How long to wait before reading again. Starts at 2 seconds for a fresh session and backs off to 5 and then 10 seconds as the session ages. `0` when the session is terminal. List results and webhook payloads also carry `0`, so check `terminal` to decide whether to stop. |
