...
| `next_poll_after_ms` | Milliseconds to wait before the next read: `2000`, rising to `5000` after 30 seconds and `10000` after 2 minutes. `0` on finished sessions, list results, and webhook payloads. Stop polling on `terminal`, not on this value. |
