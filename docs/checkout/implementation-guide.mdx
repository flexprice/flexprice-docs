...
```bash
curl -X POST https://us.api.flexprice.io/v1/customers \
  -H "x-api-key: <API_KEY>" \
...
Poll until `terminal` is `true`, waiting `next_poll_after_ms` between reads. Each read checks the session against the payment provider, so a late webhook does not hold up the result. While `terminal` is `false`, or when `stale` is `true`, show a "processing" state. Once `terminal` is `true`, show success if `checkout_status` is `completed`, and let the customer start over otherwise. See [Polling for the result](/docs/checkout/checkout-sessions#polling-for-the-result).
...
<CardGroup cols={2}>
  ...
  <Card
    icon="code"
    href="/docs/checkout/checkout-sessions"
