...
Poll until `terminal` is `true`, waiting `next_poll_after_ms` between reads. Each read checks the session against the payment provider, so a late webhook does not hold up the result. Once `terminal` is `true`, `checkout_status` holds the final result: `completed`, `failed`, or `expired`. See [Polling for the result](/docs/checkout/checkout-sessions#polling-for-the-result).
