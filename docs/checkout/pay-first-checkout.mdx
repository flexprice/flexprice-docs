...
Poll `GET /v1/checkout/sessions/{id}` from your redirect pages until `terminal` is `true`, then read `checkout_status` before showing the customer a result. A declined payment leaves the session `pending` so the customer can retry. See [Polling for the result](/docs/checkout/checkout-sessions#polling-for-the-result).
