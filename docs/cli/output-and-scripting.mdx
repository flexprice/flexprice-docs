...
## How the CLI splits stdout and stderr
...
<CardGroup cols={3}>
  <Card icon="/images/developers/icons/spinner.svg" title="Progress">
    Animation is suppressed when stderr is not a terminal, under `TERM=dumb`, and under `--quiet`.
  </Card>
  <Card icon="/images/developers/icons/palette.svg" title="Colour">
    Disabled when `NO_COLOR` is set, under `TERM=dumb`, and with `--no-color`.
  </Card>
  <Card icon="/images/developers/icons/keyboard.svg" title="Prompts">
...
## Confirming destructive CLI commands
