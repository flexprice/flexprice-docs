...
| Invalid URL or 404       | The server uses `BASE_URL` + path. Set base URL to `https://api.cloud.flexprice.io/v1` (include `/v1`, no trailing slash). For npx: use `--server-url https://us.api.flexprice.io/v1`. For local/Docker: set `BASE_URL` in `.env` or the MCP config `env` object. |
