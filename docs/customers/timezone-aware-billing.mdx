...
curl -X PATCH https://us.api.flexprice.io/v1/customers/{id} \
  -H "Authorization: Bearer <API_KEY>" \
  -H "Content-Type: application/json" \
  -d '{
...
<Warning>
  Updating a customer's timezone does **not** affect active subscriptions. Only
  new subscriptions created after the update will use the new timezone.
</Warning>
...
<Note>If no timezone is selected, the customer defaults to `UTC`.</Note>
...
<CardGroup cols={2}>
  ...
  <Card
    icon="refresh"
    href="/docs/subscriptions/customers-create-subscription"
  ...
  <Card
    icon="tag"
    href="/docs/product-catalogue/coupons/overview"
    title="Coupons"
  >
