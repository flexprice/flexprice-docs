---
title: "Pricing Widget"
description: "Complete pricing selector with billing toggle, plan grid, CTAs, and recommended plans"
---

The **Pricing Widget** is the full pricing selector experience. Use it when you want a ready-made pricing page rather than composing table and cards yourself.

It composes header, billing period toggle, a grid of [Pricing Cards](/docs/exportable-ui/pricing-widget/pricing-card), feature comparison, and footer actions. See the [architecture](/docs/exportable-ui/overview#component-architecture) on the overview.

## Features

- Monthly / yearly (or custom) billing toggle
- Multiple plans
- Usage-based, hybrid, and enterprise plans
- CTA buttons
- Feature comparison
- Highlight recommended plans
- Custom branding via theme / CSS variables

## Best used for

- Marketing websites and landing pages
- Customer portal
- Product onboarding

## Import

```tsx
import '@flexprice/flexprice-ui/style.css';
import { PricingWidget } from '@flexprice/flexprice-ui';
```

## Usage

```tsx
<PricingWidget
  plans={plans}
  onPlanSelect={handlePlanSelect}
  onCheckout={handleCheckout}
  onContactSales={handleContactSales}
/>
```

Pass presentational `Plan` data from the Flexprice SDK, REST, your backend, or static JSON. See [Ways to provide data](/docs/exportable-ui/overview#ways-to-provide-data).

Wire callbacks into [Checkout](/docs/checkout/overview) or your own payment flow.

## Map Flexprice API data

```ts
import { adaptPlanToCard, filterAndSortPlans } from '@flexprice/flexprice-ui';

const plans = filterAndSortPlans(plansWithData, 'USD', 'MONTHLY').map((p) =>
  adaptPlanToCard(p, grants)
);
```

## See also

<Card icon="table" horizontal={true} href="/docs/exportable-ui/pricing-widget/pricing-table" title="Pricing Table" />

<Card icon="table" horizontal={true} href="/docs/exportable-ui/pricing-widget/pricing-card" title="Pricing Card" />

<Card icon="book-open" horizontal={true} href="/docs/exportable-ui/overview" title="Exportable UI overview" />
