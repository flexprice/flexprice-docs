...
<Steps>
  ...
  <Step title="Add the custom expression">
    ...
    <Frame>
      ![Aggregation section with the Custom expression toggle highlighted](/images/docs/product-catalogue/features/custom-expression/switch-from-field-to-cel.png)
    </Frame>

    Enter a CEL formula in the **Custom Expression** field. Variables are the top-level property names on your events.

    <Frame>
      ![Custom Expression field with ceil(duration_ms / 1000) and inline help text listing available functions](/images/docs/product-catalogue/features/custom-expression/input-cel.png)
    </Frame>
  </Step>
