...
Evaluation runs after usage events arrive (debounced, roughly five minutes by default). When a window's usage reaches its quota, the grant flips to `exhausted`, one alert log is written, and a webhook fires. Recovery is not a state transition: a fresh window with a fresh quota simply opens when the next one starts.
