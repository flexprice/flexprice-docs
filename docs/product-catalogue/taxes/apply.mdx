...
description: "Link tax rates to tenants, customers, and subscriptions. Manage post-creation changes via the subscription modification API."
---
...
curl -X POST https://api.flexprice.io/v1/subscriptions/<subscription_id>/modify/execute \
  ...
  -d '{
    ...
    "tax_params": {
      "action": "remove",
      "association_id": "<tax_association_id>"
    }
  }'
```

Get the `association_id` from `GET /v1/taxes/associations?entity_type=subscription&entity_id=<subscription_id>`.
...
| `association_id` | string | When `action=remove` | ID of the tax association to detach |
...
```bash
curl -X PUT https://api.flexprice.io/v1/taxes/associations/<association_id> \
  -H "x-api-key: <API_KEY>" \
  -H "Content-Type: application/json" \
  -d '{
    "auto_apply": false
...
Use the modification API with `action: "remove"`. Pass the `association_id` from listing the subscription's associations.
