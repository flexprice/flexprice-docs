...
| `all_reader` | Your service needs to read every resource but never change anything (read-only API keys) |
...
```json
{
  "id": "user_01M2SXPDPM7NZJGQBH9H0905QS",
  "name": "Event Ingestion Worker",
  "type": "service_account",
  "roles": ["event_ingestor"],
  "tenant": {
...
Then [create an API key](/docs/rbac/api#for-a-service-account) with `service_account_id` set to the returned `id`. The key inherits `all_reader`.
...
<ParamField path="roles" type="array" required>
  ...
  `all_writer` cannot be assigned to a service account.
</ParamField>
