...
- **`custom_currencies`**: Optional. Map keyed by currency code. When empty, only Flexprice's supported fiat currencies are accepted, therefore a custom code must be configured here before anything can be created in it.
