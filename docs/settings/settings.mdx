...
- **`critical`**, **`warning`**, **`info`** — Optional. Each has `threshold` (string, e.g. `"0"`, `"-5"`, `"10"`, or a percentage like `"20"` when `alert_threshold_type` is `"percentage"`) and `condition` (`"below"` or `"above"`).
