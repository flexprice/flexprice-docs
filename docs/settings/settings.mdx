...
- **`alert_threshold_type`** — Optional. Defaults to `"absolute"` when omitted. One threshold type applies to all three levels, so Critical, Warning and Info are always read the same way.
...
- **`critical`**, **`warning`**, **`info`** — Optional. Each has:
  ...
  - **`condition`** — `"below"` or `"above"`. Wallet balance alerts use `"below"`.
