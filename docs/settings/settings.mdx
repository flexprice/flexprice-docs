...
- **`alert_threshold_type`** — Optional. `"absolute"` or `"percentage"`. Defaults to `"absolute"` when omitted. Applies to all three levels together; a config cannot mix threshold types across levels. See [Threshold Type: Absolute or Percentage](/docs/wallet/low-balance-alert#threshold-type-absolute-or-percentage) for how percentage thresholds are calculated.
- **`critical`**, **`warning`**, **`info`** — Optional. Each has `threshold` (string, e.g. `"0"`, `"-5"`, `"10"`, or a percentage like `"20"` when `alert_threshold_type` is `"percentage"`) and `condition` (`"below"` or `"above"`). The dashboard writes `"below"` for wallet balance alerts; `"above"` is accepted by the API only.

<Note>
  A tenant default set with `alert_threshold_type: "percentage"` still applies only to ongoing balance alerts. See [Threshold Type: Absolute or Percentage](/docs/wallet/low-balance-alert#threshold-type-absolute-or-percentage) for what this means for wallets using this default.
</Note>
