...
- **Optional: Add addons** - After a plan is selected, an **Addons** section appears in the form. Use it to attach addons with per-charge quantities and price overrides. See [Add Addons to a Subscription](/docs/product-catalogue/addons/add-to-subscription) for detailed information.
...
- For example, a customer can have both a `Basic Plan` and a `Premium Support` subscription, each billed separately. To bill optional extras on the same subscription and invoice instead, attach an [addon](/docs/product-catalogue/addons/overview).
