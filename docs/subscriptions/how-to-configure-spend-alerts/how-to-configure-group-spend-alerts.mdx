...
Group spend alerts monitor the **usage-based spend summed across every line item on a subscription whose feature belongs to a given group**, for example every metered feature grouped under "Compute" or "Storage". This scope is useful when several distinct meters should count toward one shared budget.

<Note>
Group spend alerts are configured via API only. There is no dashboard screen for this scope yet. Subscription and line item scopes do have a dashboard UI; see [Subscription spend alerts](/docs/subscriptions/how-to-configure-spend-alerts/how-to-configure-subscription-spend-alerts) and [Line item spend alerts](/docs/subscriptions/how-to-configure-spend-alerts/how-to-configure-subscription-line-item-spend-alerts).
</Note>
...
| Level | Purpose |
|-------|---------|
| **Info** | Informational alert when the group's summed spend crosses a defined threshold |
...
- **Resolves group membership**: On every usage event affecting the subscription, resolves which of the subscription's line items map (via their meter's feature) to this group
- **Sums spend**: Adds up the usage-based spend of every line item in the group, using the same invoicing-grade calculation used to generate invoices
- **Compares against thresholds**: Evaluates the summed total against Critical, Warning, and Info
- **Notifies**: Sends a webhook only when the state changes

<Info>
Only groups actually touched by an event's usage are re-evaluated for that event. A group with no configured alert, or whose member features weren't touched by the event, is skipped without extra computation.
</Info>
...
<Check>
**Verify group membership before debugging a silent alert**: if a group alert isn't firing, confirm the relevant features actually have `group_id` set to that group first.
</Check>

<Check>
**Use one group per shared budget, not per feature**: group alerts are only useful when multiple features should count toward the same limit; a single-feature group is equivalent to a line item alert.
</Check>

<Check>
**Make webhook handlers idempotent**: use the group ID plus subscription ID plus `alert_status` as an idempotency key.
</Check>
