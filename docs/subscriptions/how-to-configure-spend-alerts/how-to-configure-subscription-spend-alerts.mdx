...
| Level | Purpose |
|-------|---------|
| **Info** | Informational alert when total spend crosses a defined threshold |
| **Warning** | Early warning level before critical |
| **Critical** | Critical alert level requiring immediate action |

Each level is configured with a **threshold value** (in the subscription's currency) and always uses the **above** condition, so spend alerts fire when spend rises above the threshold.

Alerts are turned on or off using **Enable Alerts** (`alert_enabled` in the API).

## Configuring via the Dashboard

<Steps>
  <Step title="Open the subscription">
    - Go to **Billing** → **Subscriptions**, or open a customer and select their subscription
    - You can do this from either the **Subscription Details** page or the customer's **Overview** tab subscriptions list
  </Step>
  ...
  <Step title="Enable alerts">
    In the **Subscription Alert Settings** dialog, turn **Enable Alerts** on to activate monitoring against your configured thresholds.
  </Step>
...
<Info>
Evaluation happens inline as usage events are ingested. There is no polling delay. The very first check for a new alert setting is silent if spend is already below every threshold (state `ok`); no webhook fires until spend actually crosses a level.
</Info>
...
- **Account health monitoring**: Route `warning` and `in_alarm` webhooks to a support or account-management queue

## Best Practices

<Check>
**Set Info and Warning with room to react**: for `above` conditions, Warning at ~80% and Critical at 100% of a target budget gives time to act before the hard limit.
</Check>

<Check>
**Combine with line item alerts**: a subscription-level alert tells you the subscription is over budget; a line item alert on the same subscription tells you which charge is driving it.
</Check>

<Check>
**Handle recovery webhooks**: when spend resets at the start of a new billing period, or a threshold is raised, Flexprice sends `alert_status: ok`. Use this to clear in-app banners or escalations.
</Check>

<Check>
**Make webhook handlers idempotent**: use the subscription ID plus `alert_status` as an idempotency key, since retried deliveries can repeat a state transition.
</Check>
