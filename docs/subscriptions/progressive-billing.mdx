...
<Note>
  Only **arrear usage charges** count toward the threshold. Fixed or advance
  charges are excluded. The invoice amount may slightly exceed the threshold
  since evaluation happens on a 5-minute interval, not in real time.
</Note>
...
<Tabs>
  <Tab title="Dashboard">
    ...
    <Frame>
      ![Progressive billing configuration: Auto invoice threshold](/images/docs/subscriptions/progressive-billing/auto-invoicing-config.png)
    </Frame>
...
<Note>
  `auto_invoice_threshold` is set at the subscription level, not the plan level.
  Each subscription can have its own threshold.
</Note>
