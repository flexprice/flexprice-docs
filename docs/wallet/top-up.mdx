...
<Steps>
  ...
  <Step title="Save">
    Click **Save Wallet** to activate auto top-up
  </Step>
</Steps>
...
- **Standard Purchased Credits** — Immediate credit addition upon payment
- **Purchased Credits (With Invoice)** — Invoice-based top-up with payment tracking

---

<Info>
  The following section documents **Purchased Credits with Invoice** flow.
</Info>
...
When you select **Purchased Credits** and enable **Generate Invoice**:

1. **Invoice Creation** — An invoice is generated with payment status `Pending`
2. **Transaction Logging** — A pending wallet transaction is created with reason `PURCHASED_CREDIT_INVOICED`
3. **Credit Hold** — Credits remain unavailable until invoice payment is confirmed

For complete transaction lifecycle details, Checkout [Purchased Credits (Invoiced)](https://docs.flexprice.io/docs/wallet/transactions#purchased-credits-invoiced) in the Wallet Transactions documentation.
...
**Payment Link**
Generate secure payment links for online transactions. Supports multiple payment gateways including Stripe. Invoice status updates automatically upon successful payment. For detailed information, see [Payment Links Documentation](https://docs.flexprice.io/integrations/stripe/payment-links).

<Warning>
  **Payment Gateway Required** — Payment recording requires a configured payment gateway. For Example, Refer to [Stripe Connection Setup](https://docs.flexprice.io/integrations/stripe/connection-setup) for configuration instructions.
</Warning>
