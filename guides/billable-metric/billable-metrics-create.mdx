...
1. **Define Aggregation**
   ...
   * **Aggregation**
     ...
     * FlexPrice supports following aggregation types (see [Aggregation Overview](/docs/Product%20catalogue/Features/Aggregation/Overview) for detailed explanations):
   ...
   * **Aggregation Value**
     ...
     * For numeric aggregations (`SUM`, `SUM WITH MULTIPLIER`, `MAX`, `WEIGHTED SUM`): specify a numeric property (e.g., `tokens`, `gb`, `response_time`).
