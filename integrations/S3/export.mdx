---
title: 'S3 Export - Automated Data Export to Amazon S3'
description: 'Automate data exports to Amazon S3 with flexible scheduling and configuration'
---

## Overview

The S3 Export feature enables automated, scheduled exports of your FlexPrice data directly to Amazon S3 buckets. This robust, enterprise-grade solution allows you to configure scheduled tasks to export data at regular intervals, trigger manual exports on-demand, and track all export operations with comprehensive execution history.
...
- **On-Demand Exports**: Trigger force runs for immediate data exports
...
|-----------|-------------|
| **Invoices** | Finalized invoices with line items, taxes, and payment details |
| **Events** | Feature usage events and billing-related activity data |

## Setting Up S3 Export

<Frame>
  ![S3 Data Exports Page](/images/docs/s3-export/s3-export-page.png)
</Frame>
...
<Frame>
  ![Create S3 Connection](/images/docs/s3-export/create-connection.png)
</Frame>
...
<Frame>
  ![Create Export Task](/images/docs/s3-export/create-export-task.png)
</Frame>
...
|------------|-------------|---------------|
| **invoice** | Finalized invoices | All invoice data, line items, taxes, payments |
...
4. **Test with Force Run**: Trigger a manual export to verify the setup

<Frame>
  ![Export Tasks List](/images/docs/s3-export/export-tasks-list.png)
</Frame>
...
<Frame>
  ![Export Details](/images/docs/s3-export/export-details.png)
</Frame>

## Force Run - Manual Export Trigger

Force Run allows you to trigger an immediate export without waiting for the next scheduled execution. This is useful for:
...
### How to Trigger a Force Run

Navigate to the export details page and click the "Force Run" button to trigger a manual export.

<Frame>
  ![Force Run Modal](/images/docs/s3-export/force-run-modal.png)
</Frame>

#### Option 1: Automatic Time Range (Default)

Trigger a force run without specifying time ranges. The system automatically calculates the export window based on the task's interval:
...
- **Independent Execution**: Force runs don't affect the regular schedule
- **Parallel Execution**: Can run alongside scheduled exports
- **Tracked Separately**: Appears in the Runs tab with "force run" or "custom" mode
...
The Runs tab displays a comprehensive history of all export executions for a scheduled task, including both automatic scheduled runs and manual force runs.

<Frame>
  ![Runs History](/images/docs/s3-export/runs-history.png)
</Frame>
...
Always test a new scheduled task with a force run before relying on automatic execution.
</Check>
...
3. Test with a force run to isolate the issue
...
- **Force Run Capability**: Trigger manual exports anytime
