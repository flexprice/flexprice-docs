...
A Moyasar connection in Flexprice stores encrypted credentials so Flexprice can sync invoices to Moyasar, generate payment links, and enable autopay via the Flexprice checkout page.
...
<Steps>
  ...
  <Step title="Configure webhook">
    Add the webhook URL in Moyasar, select `payment_paid`, and copy the Webhook Secret.
  </Step>
...
<Steps>
  <Step title="Get API Keys">
    Go to Moyasar Dashboard → Settings → API Keys. Copy both your **Secret Key** (starts with `sk_`) and your **Publishable Key** (starts with `pk_`). Both are required.
  </Step>
