...
3. **Test customer sync and checkout**: Create a test customer with address, sync an invoice, and complete a test payment via Flexprice Checkout.
4. **Monitor Webhooks**: Ensure `transaction.completed` and `subscription.activated` events are being received.
