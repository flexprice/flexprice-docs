...
| **Webhook Secret** | Whop Dashboard → **Settings** → **Webhooks** → your endpoint | Verifies incoming Whop webhooks - required to create the connection |
...
<Steps>
  ...
  <Step title="Copy the Secret and paste it into Flexprice">
    Whop shows a **Secret** for the endpoint you just created. Copy it and paste it into the **Webhook Secret** field in the Flexprice connection drawer before saving.
  </Step>
</Steps>

<Warning>
The webhook secret is required. Flexprice will not create a Whop connection without it. Without the webhook configured, invoices synced to Whop will not automatically mark as paid in Flexprice and Charge Automatically will not activate.
</Warning>
